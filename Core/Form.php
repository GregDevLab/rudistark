...
class Form
{
    ...
    private function ajoutAttributs(array $attributs): string
    {
        ...
        $courts = ['checked', 'disabled', 'readonly', 'multiple', 'required', 'autofocus', 'novalidate', 'formvalidate', 'accept'];
     ...
     * @return Form
     */
    public function debutForm(string $methode = 'post', string $action = '#', string $enctype = 'multipart/form-data', array $attributs = []): self
    {
        // On crée la balise form
        $this->formCode .= "<form action='$action' method='$methode' enctype='$enctype'";
    ...
    public function ajoutInputPrice(string $type, string $nom, array $attributs = []): self
    {
        // On ouvre la balise
        $this->formCode .= "<input type='$type' name='$nom'";
    ...
    public function addFile($nom = [], array $attributs = []): self
    {
        // On ouvre la balise
        $this->formCode .= "<input type='file' name='$nom'";

        // On ajoute les attributs
        $this->formCode .= $attributs ? $this->ajoutAttributs($attributs) : '';

        // On ajoute le texte
        $this->formCode .= 'multiple>';
